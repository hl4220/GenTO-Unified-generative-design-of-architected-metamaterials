...
# Date: 2026-05-29
# Description: Run the F1 heat-conductivity optimisation using the pretrained periodic diffusion model.
...
from valuer1 import valuer1_pass, valuer1_score
...
ckpt_dir = TASK_ROOT / f"model_ckpt_tuned_HE_max_{TEST_NUM}"
output_dir = TASK_ROOT / f"outputs_HE_max_{TEST_NUM}"
...
for iteration in range(N_ITERATIONS):
    ...
    samples_valid_cur, scores_valid_cur = collect_valid_samples(
        ...
        device=device,
        vf=VOLUME_FRACTION,
        pass_fn=valuer1_pass,
        score_fn=valuer1_score,
        batch=GEN_BATCH,
        iteration=iteration,
        useddim=False,
        explore_scale=EXPLORE_SCALE,
    ...
    with open(training_stats_path, 'a') as f:
        f.write(f'{iteration+1}\t{score_ave:.4f}\t{score_ave_preview:.4f}\n')
    ...
    print(f"Iteration {iteration+1} - Score ave: {score_ave:.4f}, Preview: {score_ave_preview:.4f}, Best: {best_score:.4f}")
