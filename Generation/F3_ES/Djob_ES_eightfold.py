...
# Date: 2026-05-29
# Description: Run the F3 eightfold-symmetric elasticity optimisation against the target tensor.
...
from model.utilities import collect_valid_samples, make_8fold_symmetric_tensor, plot_samples
from valuer3 import valuer3_pass, valuer3_score_eightfold

###############################################################################
# Model setup
###############################################################################

CKPT_PATH = MODELS_ROOT / "model_ckpt_pretrained" / "eight_fold_pretrained.pt"
...
ckpt_dir = TASK_ROOT / f"model_ckpt_tuned_ES_eightfold_{TEST_NUM}"
output_dir = TASK_ROOT / f"outputs_ES_eightfold_{TEST_NUM}"
...
for iteration in range(N_ITERATIONS):
    ...
    # Generate valid raw samples, then evaluate them after imposing eightfold symmetry.
    samples_valid_cur, scores_valid_cur = collect_valid_samples(
        ...
        score_fn=valuer3_score_eightfold,
        batch=GEN_BATCH,
        iteration=iteration,
        useddim=False,
        explore_scale=EXPLORE_SCALE,
        sample_transform_fn=make_8fold_symmetric_tensor,
    ...
    # Save the best full eightfold design found so far in both visual and loadable formats.
    best_sample = elite_samples[:1]
    best_design = make_8fold_symmetric_tensor(best_sample)
    ...
    if SAVE_SAMPLE_PREVIEWS:
        plot_samples(make_8fold_symmetric_tensor(preview_samples), save_name=preview_image_path, vf=VOLUME_FRACTION)
