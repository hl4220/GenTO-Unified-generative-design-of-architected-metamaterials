...
# Date: 2026-05-29
# Description: Run the F3 fourfold-symmetric elasticity optimisation against the target tensor.
...
from model.utilities import collect_valid_samples, make_4fold_symmetric_tensor, plot_samples
from valuer3 import valuer3_pass, valuer3_score_fourfold
...
ckpt_dir = TASK_ROOT / f"model_ckpt_tuned_ES_fourfold_{TEST_NUM}"
output_dir = TASK_ROOT / f"outputs_ES_fourfold_{TEST_NUM}"
...
for iteration in range(N_ITERATIONS):
    ...
    samples_valid_cur, scores_valid_cur = collect_valid_samples(
        ...
        score_fn=valuer3_score_fourfold,
        batch=GEN_BATCH,
        iteration=iteration,
        useddim=False,
        explore_scale=EXPLORE_SCALE,
    ...
    if SAVE_SAMPLE_PREVIEWS:
        plot_samples(make_4fold_symmetric_tensor(preview_samples), save_name=preview_image_path, vf=VOLUME_FRACTION)
